...
using TimeClock.Common.Model;
using TimeClock.Common.Responses;
using TImeClock.Function.Entities;
...
namespace TImeClock.Function.Functions
{
    public static class TimeClockApi
    {
            ...
            ILogger log)
        {
            ...
            TableQuery<TimeClockEntity> query = new TableQuery<TimeClockEntity>().Where(filter);
            TableQuerySegment<TimeClockEntity> CompletedClocks = await clockTable.ExecuteQuerySegmentedAsync(query, null);
            int deleted = 0;
            foreach (TimeClockEntity CompletedClock in CompletedClocks)
            {
            ...
            TimeClockEntity timeClockEntity = new TimeClockEntity
            {
            ...
            ILogger log)
        {
            ...
            string filter = TableQuery.GenerateFilterConditionForInt("type", QueryComparisons.Equal, 0);
            TableQuery<TimeClockEntity> query = new TableQuery<TimeClockEntity>().Where(filter);
            TableQuerySegment<TimeClockEntity> CompletedTodos = await clockTable.ExecuteQuerySegmentedAsync(query, null);
            TimeClockEntity timeClockEntity = null;
            foreach (TimeClockEntity CompletedTodo in CompletedTodos)
            {
            ...
            ILogger log)
        {
            ...
            TableQuery<TimeClockEntity> query = new TableQuery<TimeClockEntity>();
            TableQuerySegment<TimeClockEntity> clocks = await clockTable.ExecuteQuerySegmentedAsync(query, null);
        ...
        public static IActionResult getRegisterById(
            ...
            [Table("timeclock", "TIMECLOCK", "{id}", Connection = "AzureWebJobsStorage")] TimeClockEntity clockEntity,
            string id,
            ILogger log)
        {
